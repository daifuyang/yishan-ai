...
import { getMcpManager, type MCPConfig } from '../lib/mcp-manager.js';
...
const mcpRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/api/mcp/servers', async (request: FastifyRequest) => {
  ...
  fastify.patch('/api/mcp/servers/:name', async (request: FastifyRequest) => {
  ...
  fastify.delete('/api/mcp/servers/:name', async (request: FastifyRequest) => {
    const { name } = request.params as { name: string };
    await mcpManager.removeServer(name);
    return { success: true };
  });

  fastify.post('/api/mcp/servers/:name/connect', async (request: FastifyRequest) => {
    const { name } = request.params as { name: string };
    const tools = await mcpManager.connectServer(name);
    return { success: true, tools };
  });

  fastify.post('/api/mcp/servers/:name/disconnect', async (request: FastifyRequest) => {
    const { name } = request.params as { name: string };
  ...
  fastify.post('/api/mcp/call', async (request: FastifyRequest) => {
