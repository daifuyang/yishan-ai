...
interface ChatStore {
  ...
  sendMessage: (sessionId: string, content: string, model: string, mode?: 'plan' | 'build') => Promise<{ success: boolean; error?: string }>;
  retryMessage: (sessionId: string, messageId: string, model: string, mode?: 'plan' | 'build') => Promise<{ success: boolean; error?: string }>;
  subscribe: (sessionId: string) => void;
  unsubscribe: () => void;
  stopStream: (sessionId: string) => Promise<void>;
  clearMessages: () => void;
  updateMessageStatus: (messageId: string, status: 'pending' | 'success' | 'failed' | 'stopped', error?: string) => void;
      ...
      useChatStore.setState((state) => {
        ...
        return {
          messages: state.messages.map((msg) => {
            ...
            const targetIdx = msg.content.findIndex((c: ContentBlock) => c.type === 'tool_use' && !c.result);
      ...
      useChatStore.setState((state) => {
        ...
        return {
          messages: state.messages.map((msg) => {
            if (msg.id !== assistantId) return msg;
            if (!Array.isArray(msg.content)) return msg;
            const targetIdx = msg.content.findIndex((c: ContentBlock) => c.type === 'tool_use' && !c.error);
            if (targetIdx < 0) return msg;
            const updatedContent = [...msg.content];
            const targetBlock = updatedContent[targetIdx] as ContentBlock;
...
export const useChatStore = create<ChatStore>()(
  ...
  fetchMessages: async (sessionId) => {
    ...
    if (data.session?.status === 'streaming') {
      const currentMessages = get().messages;
      const localAssistant = currentMessages.find(m => m.role === 'assistant' && m.status === 'pending');
      const serverAssistant = (data.messages as Array<{ id: string; role: string; status?: string }>)?.find(m => m.role === 'assistant');
      const assistantId = localAssistant?.id || serverAssistant?.id || null;
      if (!assistantId) {
        console.warn('[fetchMessages] session is streaming but no assistant target found, fallback to non-streaming');
        set({
          isStreaming: false,
          streamingContent: '',
          isFetchingMessages: false,
        ...
        const userId = assistantIdx > 0
          ? sourceMessages.slice(0, assistantIdx).reverse().find((m) => m.role === 'user')?.id || null
          : null;
        set({
          isStreaming: true,
      ...
      set({ isStreaming: false, streamingContent: '', isFetchingMessages: false, pendingMessageId: null, activeStream: null });
    }
    } catch (error) {
      set({ isFetchingMessages: false });
    }
  ...
  sendMessage: async (sessionId, content, model, mode) => {
    ...
    set((state) => ({
      ...
      messages: [
        ...
        { id: assistantTempId, role: 'assistant', type: 'assistant', content: '', status: 'pending' as const },
      ],
    }));
        ...
        set((state) => ({
          messages: state.messages.map((msg) =>
            msg.id === userTempId || msg.id === assistantTempId
              ? { ...msg, status: 'failed' as const, error: errorData.error || `错误 ${res.status}` }
              : msg
          ),
          isStreaming: false,
          pendingMessageId: null,
      ...
      if (!res.ok) {
        ...
        set((state) => ({
          messages: state.messages.map((m) =>
            m.id === assistantTempId ? { ...m, status: 'failed' as const, error: errorData.error || `错误 ${res.status}` } : m
          ),
          isStreaming: false,
          pendingMessageId: null,
          activeStream: null,
  ...
  subscribe: (sessionId) => {
    const state = get();
    if (!state.activeStream || !state.activeStream.assistantId) {
  ...
  clearMessages: () => {
    ...
    return set({ messages: [], isStreaming: false, isFetchingMessages: false, streamingContent: '', pendingMessageId: null, activeStream: null });
  },
  ...
  rollbackMessage: async (sessionId, messageId) => {
    ...
    set({ messages: newMessages, isStreaming: false, streamingContent: '', pendingMessageId: null, activeStream: null });
