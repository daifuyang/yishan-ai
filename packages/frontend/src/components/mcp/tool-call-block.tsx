...
import type { ToolCall, ToolCallGroup as ToolCallGroupType } from '@/types';
    ...
    <div className="flex items-center gap-3">
      ...
      {contextTools.reads > 0 && (
        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>
            {contextTools.reads} read{contextTools.reads !== 1 ? 's' : ''}
          </span>
        </span>
      )}
      {contextTools.searches > 0 && (
        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>
            {contextTools.searches} search{contextTools.searches !== 1 ? 'es' : ''}
          </span>
        </span>
      )}
      {contextTools.lists > 0 && (
        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>
            {contextTools.lists} list{contextTools.lists !== 1 ? 's' : ''}
          </span>
        </span>
      )}
      {shellTools.length > 0 && (
        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>
            {shellTools.length} shell{shellTools.length !== 1 ? 's' : ''}
          </span>
        </span>
      )}
      {otherTools.length > 0 && (
        <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <span>
            {otherTools.length} other{otherTools.length !== 1 ? 's' : ''}
          </span>
        </span>
      )}
    </div>
      ...
      <span className="tool-shell-submessage-width">
        <span className="tool-shell-submessage-text">
          <span
            ...
            style={
              animate ? { opacity: 0, filter: 'blur(2px)' } : { opacity: 1, filter: 'blur(0px)' }
            }
          >
            {text}
          </span>
...
function BashTool({ toolCall }: { toolCall: ToolCall }) {
  ...
  const output =
    typeof toolCall.output === 'string' ? toolCall.output : formatJson(toolCall.output);
          ...
          <div className="tool-card-info">
            <div className="tool-card-info-structured">
              <div className="tool-card-info-main">
                <TextShimmer text="Shell" active={pending} className="tool-card-title" />
                {!pending && displayText && (
      ...
      >
        <div className="tool-card-trigger-content">
          <div className="tool-card-info">
            <div className="tool-card-info-structured">
              <div className="tool-card-info-main">
                <TextShimmer text="Read" active={pending} className="tool-card-title" />
                {!pending && fileName && <span className="tool-card-subtitle">{fileName}</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      >
        <div className="tool-card-trigger-content">
          <div className="tool-card-info">
            <div className="tool-card-info-structured">
              <div className="tool-card-info-main">
                <TextShimmer text="List" active={pending} className="tool-card-title" />
                {!pending && path && <span className="tool-card-subtitle">{dir || '/'}</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      >
        <div className="tool-card-trigger-content">
          <div className="tool-card-info">
            <div className="tool-card-info-structured">
              <div className="tool-card-info-main">
                <TextShimmer text="Glob" active={pending} className="tool-card-title" />
                {!pending && (
      ...
      >
        <div className="tool-card-trigger-content">
          <div className="tool-card-info">
            <div className="tool-card-info-structured">
              <div className="tool-card-info-main">
                <TextShimmer text="Search" active={pending} className="tool-card-title" />
                {!pending && (
                  <span className="tool-card-subtitle">
                    {dir || '/'}
                    {pattern ? ` - ${pattern}` : ''}
...
function GenericToolItem({ toolCall }: { toolCall: ToolCall }) {
  const [expanded, setExpanded] = useState(false);
  const [copied, setCopied] = useState(false);
  const _Icon = getToolIcon(toolCall.name);
      ...
      >
        <div className="tool-card-trigger-content">
          <div className="tool-card-info">
            <div className="tool-card-info-structured">
              <div className="tool-card-info-main">
                <TextShimmer text={label} active={pending} className="tool-card-title" />
                {!pending && description && (
    ...
    <div className="tool-block">
      ...
      >
        ...
        <div className="tool-block-header-actions">
          {runningCount > 0 && (
            <span className="text-xs text-blue-500 animate-pulse">{runningCount} running...</span>
          )}
...
export function groupToolCalls(toolCalls: ToolCall[]): ToolCallGroupType[] {
  const groups: ToolCallGroupType[] = [];
...
export function ContextToolGroup({
  ...
  return (
    <div className="tool-group">
      <button
        type="button"
        className="tool-group-header w-full text-left"
        onClick={() => setExpanded(!expanded)}
      >
        <div className="flex items-center gap-3">
...
export function IndependentToolGroup({
  toolCalls,
  defaultExpanded = false,
}: {
  toolCalls: ToolCall[];
...
export function ToolCallGroup({
  ...
  const runningCount = allTools.filter(
    (t) => t.status === 'pending' || t.status === 'running'
  ).length;
          ...
          >
            <div className="tool-group-content-inner">
              {groups.map((group, _groupIdx) =>
                group.tools.map((tool) => <ToolItem key={tool.id} toolCall={tool} />)
              )}
            </div>
          </motion.div>
        )}
